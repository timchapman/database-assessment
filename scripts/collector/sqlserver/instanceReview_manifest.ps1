...
elseif (((checkStringForSpecialChars -inputString $manualUniqueId) -eq "fail") -and (![string]::IsNullorEmpty($manualUniqueId))) {
    ...
    if (([string]::IsNullorEmpty($port)) -or ($port -eq "default")) {
        ...
        $folderObj = sqlcmd -S $serverName -i sql\foldername.sql -d master -C -l 30 -W -m 1 -u -w 32768 -v database=$databaseNameFilter @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' }
        ...
        if ([string]$database -ne "all") {
            $validDBObj = sqlcmd -S $serverName -i sql\checkValidDatabase.sql -C -l 30 -W -m 1 -u -h-1 -w 32768 -v database=$databaseNameFilter @sqlcmdAuthArgs | Where-Object { $_ -notmatch '^-' }
...
if ($folderObj -is [System.Array]) {
    $dataLine = $folderObj[-1]
} else {
    $dataLine = $folderObj
}
...
$foldername = $machinename + '_' + $current_ts
$logFileShort = 'collector_log.log'
$logFileLong = 'opdb' + '__' + 'CollectorLog' + '__' + $dbversion + '_' + $op_version + '_' + $machinename + '_' + $dbname + '_' + $instancename + '_' + $current_ts + '.log'
$logFile = $logFileShort

$sqlErrorLogFileShort = 'sql_validation_log.log'
$sqlErrorLogFileLong = 'opdb' + '__' + 'SqlValidationLog' + '__' + $dbversion + '_' + $op_version + '_' + $machinename + '_' + $dbname + '_' + $instancename + '_' + $current_ts + '.log'
...
$deepScanner      = $deepScannerShort
$computerSpecsFileShort = 'machine_specs.csv'
$computerSpecsFileLong = 'opdb' + '__' + 'DbMachineSpecs' + $outputFileSuffix
$computerSpecsFile = $computerSpecsFileShort

$cpuUsageFileNameLong  = 'opdb' + '__' + 'CpuUsage' + $outputFileSuffix
$cpuUsageFileName = $cpuUsageFileNameShort

$memoryUsageFileNameLong  = 'opdb' + '__' + 'MemoryUsage' + $outputFileSuffix
$memoryUsageFileName      = $memoryUsageFileNameShort
...
$tranLogBkupCountByDayByHour = $tranLogBkupCountByDayByHourShort
...
$tranLogBkupSizeByDayByHour = $tranLogBkupSizeByDayByHourShort
...
$databaseLevelBlockingFeatures = $databaseLevelBlockingFeaturesShort
...
sqlcmd -S $serverName -i sql\componentsInstalled.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$compFileNameShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\serverProperties.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$srvFileNameShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\dbServerUnsupportedFlags.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$dbServerFlagsShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\dbServerFeatures.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$blockingFeaturesShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\linkedServersDetail.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$linkedServersShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\dbClusterNodes.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$dbClusterNodesShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\dbccTraceFlags.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$dbccTraceFlgShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\cpuUsage.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$cpuUsageFileNameShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\memoryUsage.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$memoryUsageFileNameShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\batchFootprint.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$batchFootprintFileNameShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\waitsStats.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$waitsStatsShort -Encoding utf8
...
Set-Content -Path $foldername\$queryOptimizerInfoShort -Encoding utf8 -Value '"PKEY"|"name"|"value"|"is_default"|"dma_source_id"|"dma_manual_id"'
sqlcmd -S $serverName -i sql\queryOptimizerInfo.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$queryOptimizerInfoShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\fileIoLatency.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$fileIoLatencyShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\instanceErrorLog.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$instanceErrorLogShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\deprecatedFeatures.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$deprecatedFeaturesShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\diskVolumeInfo.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$diskVolumeInfoShort -Encoding utf8
...
sqlcmd -S $serverName -i sql\dbServerConfigurationSettings.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$dbServerConfigShort -Encoding utf8
...
if ($isCloudOrLinuxHost -eq "AZURE") {
    ...
    Set-Content -Path $foldername\$tranLogBkupCountByDayByHourShort -Encoding utf8 -Value '"PKEY"|"collection_date"|"day_of_month"|"total_logs_generated"|"h0_count"|"h1_count"|"h2_count"|"h3_count"|"h4_count"|"h5_count"|"h6_count"|"h7_count"|"h8_count"|"h9_count"|"h10_count"|"h11_count"|"h12_count"|"h13_count"|"h14_count"|"h15_count"|"h16_count"|"h17_count"|"h18_count"|"h19_count"|"h20_count"|"h21_count"|"h22_count"|"h23_count"|"avg_per_hour"|"dma_source_id"|"dma_manual_id"'
    Set-Content -Path $foldername\$tranLogBkupSizeByDayByHourShort -Encoding utf8 -Value '"PKEY"|"collection_date"|"day_of_month"|"total_logs_generated_in_mb"|"h0_size_in_mb"|"h1_size_in_mb"|"h2_size_in_mb"|"h3_size_in_mb"|"h4_size_in_mb"|"h5_size_in_mb"|"h6_size_in_mb"|"h7_size_in_mb"|"h8_size_in_mb"|"h9_size_in_mb"|"h10_size_in_mb"|"h11_size_in_mb"|"h12_size_in_mb"|"h13_size_in_mb"|"h14_size_in_mb"|"h15_size_in_mb"|"h16_size_in_mb"|"h17_size_in_mb"|"h18_size_in_mb"|"h19_size_in_mb"|"h20_size_in_mb"|"h21_size_in_mb"|"h22_size_in_mb"|"h23_size_in_mb"|"avg_mb_per_hour"|"dma_source_id"|"dma_manual_id"'
    sqlcmd -S $serverName -i sql\dbServerTranLogBackupCountByDayByHour.sql -d msdb -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$tranLogBkupCountByDayByHourShort -Encoding utf8
Update-DmaManifest -shortName $tranLogBkupCountByDayByHourShort -longName $tranLogBkupCountByDayByHourLong
    sqlcmd -S $serverName -i sql\dbServerTranLogBackupSizeByDayByHour.sql -d msdb -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$tranLogBkupSizeByDayByHourShort -Encoding utf8
...
foreach ($databaseName in $dbNameArray) {
    ...
    if ($databaseName -inotmatch "tempdb") {
        ...
        sqlcmd -S $serverName -i sql\objectList.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$objectListShort -Encoding utf8
Update-DmaManifest -shortName $objectListShort -longName $objectListLong
        ...
        sqlcmd -S $serverName -i sql\tableList.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$tableListShort -Encoding utf8
Update-DmaManifest -shortName $tableListShort -longName $tableListLong
        ...
        sqlcmd -S $serverName -i sql\indexList.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$indexListShort -Encoding utf8
Update-DmaManifest -shortName $indexListShort -longName $indexListLong
        ...
        sqlcmd -S $serverName -i sql\columnDatatypes.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$columnDatatypesShort -Encoding utf8
Update-DmaManifest -shortName $columnDatatypesShort -longName $columnDatatypesLong
        ...
        sqlcmd -S $serverName -i sql\userConnectionInfo.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$userConnectionListShort -Encoding utf8
Update-DmaManifest -shortName $userConnectionListShort -longName $userConnectionListLong
        ...
        sqlcmd -S $serverName -i sql\dbServerFeaturesDatabaseLevel.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$databaseLevelBlockingFeaturesShort -Encoding utf8
Update-DmaManifest -shortName $databaseLevelBlockingFeaturesShort -longName $databaseLevelBlockingFeaturesLong
    ...
    sqlcmd -S $serverName -i sql\dbSizes.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$dbsizesShort -Encoding utf8
Update-DmaManifest -shortName $dbsizesShort -longName $dbsizesLong
        ...
        sqlcmd -S $serverName -i sql\tsqlPostgresDeepScanner.sql -d $databaseName -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$deepScannerShort -Encoding utf8
...
if ($isCloudOrLinuxHost -eq "AZURE") {
    ...
    sqlcmd -S $serverName -i sql\tableList.sql -d tempdb -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey database=$databaseName dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$tableListShort -Encoding utf8
Update-DmaManifest -shortName $tableListShort -longName $tableListLong
}
...
if ($isCloudOrLinuxHost -eq "AZURE") {
    ...
    sqlcmd -S $serverName -i sql\azureDbResourceStats.sql -d $dbname -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$azureDbResShort -Encoding utf8
    ...
    sqlcmd -S $serverName -i sql\azureResourceStats.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$azureResShort -Encoding utf8
    ...
    sqlcmd -S $serverName -i sql\azureResourceGovernance.sql -d $dbname -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$azureGovShort -Encoding utf8
    ...
    sqlcmd -S $serverName -i sql\azureElasticPoolStats.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$azurePoolShort -Encoding utf8
    Update-DmaManifest -shortName $azurePoolShort -longName $azurePoolLong
}

<# Getting HW Specs via Native T-SQL DMVs (Cross-platform) #>
...
sqlcmd -S $serverName -i sql\machineSpecsTsql.sql -d master -C -l 30 -W -m 1 -u -h-1 -w 32768 -v pkey=$pkey dmaSourceId=$dmaSourceId dmaManualId=$manualUniqueId -s"|" @sqlcmdAuthArgs | Where-Object { $_ -notmatch '---' } | Add-Content -Path $foldername\$computerSpecsFileShort -Encoding utf8
